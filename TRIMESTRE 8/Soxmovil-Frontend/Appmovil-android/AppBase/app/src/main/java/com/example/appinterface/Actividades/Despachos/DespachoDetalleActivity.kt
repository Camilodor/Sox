...
class DespachoDetalleActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnGuardar.setOnClickListener {
            val despachoEditado = Despachos(
                id = despacho?.id ?: 0,
                mercancias_id = etMercanciaH.text.toString().toInt(),
                vehiculos_id = etVehiculos.text.toString().toInt(),
                usuarios_id = etUsuariosH.text.toString().toInt(),
                tipo_pago_id = etTipopagoH.text.toString().toIntOrNull() ?: 1,
                fecha_despacho = etFechaH.text.toString(),
                negociacion = etNegociacion.text.toString().toDoubleOrNull() ?: 0.0,
                anticipo = etAnticipo.text.toString().toDoubleOrNull() ?: 0.0,
                saldo = etSaldo.text.toString().toDoubleOrNull() ?: 0.0,
                observaciones_mer = etObservacionesH.text.toString(),


            )

            Log.d("DespachoDetalle", "Intentando actualizar despacho ID=${despachoEditado.id}")

            despachoEditado.id?.let { it1 ->
                api.updateDespacho(it1, despachoEditado)
                    .enqueue(object : Callback<ResponseBody> {
                        override fun onResponse(
                            ...
                            if (response.isSuccessful) {
                                Log.d("DespachoDetalle", "Despacho actualizado correctamente")
                                Toast.makeText(
                                    this@DespachoDetalleActivity,
                                    "Despacho actualizado correctamente",
                                    Toast.LENGTH_SHORT
                                ).show()
                                finish()
                            } else {
                                Log.e("DespachoDetalle", "Error al actualizar: ${response.code()}")
                                Toast.makeText(
                                    this@DespachoDetalleActivity,
                                    "Error al actualizar",
                                    Toast.LENGTH_SHORT
                        ...
                        override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                            Log.e("DespachoDetalle", "Fallo red: ${t.message}")
                            Toast.makeText(
                                this@DespachoDetalleActivity,
                                "Error: ${t.message}",
                                Toast.LENGTH_SHORT
