...
import com.example.demo.usuarios.ConexionService
...
class ConexionController {
    ...
    @GetMapping("/usuarios")
    fun obtenerUsuarios(): List<Usuarios> = conexionService.obtenerUsuarios()

    // SHOW → GET /usuarios/{id}
    @GetMapping("/usuarios/{id}")
    fun obtenerUsuario(@PathVariable id: Int): Usuarios? =
        conexionService.obtenerUsuarioPorId(id)

    // STORE → POST /usuarios
    @PostMapping("/usuarios")
    fun crearUsuario(@RequestBody usuario: Usuarios): String {
        val filas = conexionService.crearUsuario(usuario)
    ...
    fun actualizarUsuario(@PathVariable id: Int, @RequestBody usuario: Usuarios): String {
        val filas = conexionService.actualizarUsuario(id, usuario)
    ...
    fun eliminarUsuario(@PathVariable id: Int): String {
        val filas = conexionService.eliminarUsuario(id)
